...
int main()
{
    ...
    int N = 1500;                       // 数据点。什么作用？？？ 此处由100改为1500,增大观测数
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // 观测 y
//        double y = std::exp( a*x*x + b*x + c ) + n; // 此处换为y=a*x*x + b*x + c
        double y = a*x*x + b*x + c + n;

        // 每个观测对应的残差函数 edge
        shared_ptr< CurveFittingEdge > edge(new CurveFittingEdge(x,y));
        std::vector<std::shared_ptr<Vertex>> edge_vertex;
        edge_vertex.push_back(vertex); // 顶点，系统要求的变量a b c
    ...
    std::cout << vertex->Parameters().transpose() << std::endl; // 优化之后的值
