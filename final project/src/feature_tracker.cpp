...
void FeatureTracker::ExtractEdgeFeature(std::vector<cv::Point2f> &total_pts, int NeedNum)
{
    ...
    cv::Mat gradx = cv::Mat::zeros(forw_img.rows, forw_img.cols, CV_32F); // x方向梯度
    cv::Mat grady = cv::Mat::zeros(forw_img.rows, forw_img.cols, CV_32F); // y方向梯度
    cv::Mat mag = cv::Mat::zeros(forw_img.rows, forw_img.cols, CV_32F);

    cv::GaussianBlur(forw_img, img, cv::Size(3, 3), 0, 0); // 高斯滤波
    ...
    for(int x=1; x + MIN_DIST < forw_img.cols - 1; x = x + MIN_DIST)
    {
        for(int y = 1; y + MIN_DIST < forw_img.rows - 1; y = y + MIN_DIST)
        {
            float max_grad = 0;
            int maxgrad_x = 0;
            int maxgrad_y = 0;
